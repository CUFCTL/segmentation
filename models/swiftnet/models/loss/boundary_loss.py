"""PyTorch implementation of BoundaryAwayFocalLoss function 
described in: https://arxiv.org/abs/1905.08929
"""
import torch
...
class BoundaryAwareFocalLoss(nn.Module):
    ...
    def forward(self, input, target, batch, **kwargs):
        if input.shape[-2:] != target.shape[-2:]: # If [H, W] are the same - ignores [B] and [C]
        ...
        label_distance_alphas = batch['label_distance_alphas'].to(input.device) # Comes from the LabelDistanceTransforms Class
        ...
        if input.dim() > 2:
            ...
            input = input.contiguous().view(-1, input.size(2))  # B,H*W,C => B*H*W,C
