"""Final Model file that puts together the backbone (modified resnet18)
...
class SemsegModel(nn.Module):
    def __init__(self, backbone, num_classes, num_inst_classes=None, use_bn=True, k=1, bias=True,
        ...
        self.logits = logit_class(
            self.backbone.num_features, self.num_classes, batch_norm=use_bn, k=k, bias=bias)
    ...
    """################################################################################
       HAVING MULTIPLE INPUTS WAS CAUSING SO MUCH TROUBLE DURING ONNX INFERENCE
       SO CURRENTLY I AM MAKING THIS JUST ONE INPUT
       ################################################################################
    """
    #def forward(self, image, target_size, image_size):
    #def forward(self, image, image_size):
    def forward(self, image):
        image_size = image.shape[2:4]
        #print(image_size)
        #print(target_size)
        ...
        if hasattr(self, 'border_logits'):
            additional['border_logits'] = self.border_logits(
                features).sigmoid()
    ...
    def prepare_data(self, batch, image_size, device=torch.device('cuda'), img_key='image'):
        ...
        #image = batch[img_key].detach().requires_grad_(
        #    self.img_req_grad).to(device)
        image = batch[img_key].to(device)
        return {
            'image': image
    ...
    def loss(self, batch):
        """Predict image and calculate loss during training loop

        Args:
            batch: Dictionary of raw images and labels. Batch comes from the training loop.
        """
        assert self.criterion is not None
        labels = batch['labels'].cuda()
        # Logits shape of [14, 19, 768, 768] - batch, num_class (channels), crop_h, crop_w
        # Labels shape of [14, 768, 768]
        logits, additional = self.do_forward(
            batch, image_size=labels.shape[-2:])
    ...
    def ms_forward(self, batch, image_size=None):
        image_size = batch.get(
            'target_size', image_size if image_size is not None else batch['image'].shape[-2:])
        ms_logits = None
