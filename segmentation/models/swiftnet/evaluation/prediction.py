...
__all__ = ['StorePreds', 'StoreSubmissionPreds', 'ApplyColorMap']


class StorePreds:
    """Applies color map to predicted image and stacks the original image (top),
    the predicted segmented image (middle), and the ground truth segmented image (bottom)
    into one picture. The color map is applied using the ColorizeLabels() class.
    """
...
class ApplyColorMap:
    ...
    into one picture. The color map is applied using the ColorizeLabels() class.
    """
    def __init__(self, to_img, to_color):
        self.to_img = to_img
        self.to_color = to_color
    ...
    def __call__(self, pred):
        colored_img = self.to_color(pred)
        return colored_img